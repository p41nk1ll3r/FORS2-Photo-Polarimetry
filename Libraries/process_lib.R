...
################################################################################
# Creates a plot of 'x_dat' vs 'y_dat' and saves it to a png file.             #
# If 'fit_bool' is true, it will attempt to fit 'x_dat' to 'y_dat' and         #
# if it succeeds it will also create a plot of it and save it to a pnf file.   #
################################################################################
fit_n_plot_to_png <- function(x_dat, y_dat, x_unc_n, x_unc_p = x_unc_n,        #
                              y_unc_n, y_unc_p = y_unc_n, arrow_bool = T,      #
                              no_fit_plot_output_path, fit_bool = F,           #
                              fit_plot_output_path = NULL, min_x = NA,         #
                              max_x = NA, min_y = NA, max_y = NA,              #
                              log_x_bool = F, log_y_bool = F,                  #
                              lim_marg_x = 0.05, lim_marg_y = 0.1, png_width,  #
                              png_height, plot_type = "p", plot_pch = 18,      #
                              plot_lty = 1, plot_cex = 1.2, x_lab = "x-axis",  #
                              y_lab = "y-axis", arrow_typ = 3,                 #
                              arrow_col = "snow4", fit_col = "#1C86EE",        #
                              fit_lty = 2, fit_lwd = 2, fit_unc_col = "red",   #
                              fit_unc_lty = 3, fit_unc_lwd = 1, fit_bty = "n", #
                              leg_xpos = NA, leg_ypos = NA){                   #
                                                                               #
  if(length(x_dat) != length(y_dat)){                                          #
    print(paste0("ERROR: 'x_dat' and 'y_dat' must have the same length. ",     #
                 "'x_dat' has length ", length(x_dat), " and 'y_dat' has ",    #
                 "length ", length(y_dat), ". Returning NULL."))               #
    return(NULL)                                                               #
  }                                                                            #
  if(length(x_unc_n) != length(x_dat) | length(y_unc_n) != length(x_dat)){     #
    print(paste0("ERROR: 'x_unc_n' and 'y_unc_n' must have the same length as",#
                 " 'x_dat'. 'x_dat' has length ", length(x_dat), ", 'x_unc_n'",#
                 " has length ", length(x_unc_n), " and 'y_unc_n' has length ",#
                 length(y_unc_n), ". Returning NULL."))                        #
    return(NULL)                                                               #
  }                                                                            #
  if(length(x_unc_p) != length(x_dat) | length(y_unc_p) != length(x_dat)){     #
    print(paste0("ERROR: 'x_unc_p' and 'y_unc_p' must have the same length as",#
                 " 'x_dat'. 'x_dat' has length ", length(x_dat), ", 'x_unc_p'",#
                 " has length ", length(x_unc_p), " and 'y_unc_p' has length ",#
                 length(y_unc_p), ". Returning NULL."))                        #
    return(NULL)                                                               #
  }                                                                            #
                                                                               #
  x_nas <- which(is.na(x_dat), arr.ind = T)                                    #
  y_nas <- which(is.na(y_dat), arr.ind = T)                                    #
                                                                               #
  nas <- union(x_nas, y_nas)                                                   #
                                                                               #
  if(length(nas) != 0){                                                        #
    y_dat <- y_dat[-nas]                                                       #
    y_unc_n <- y_unc_n[-nas]                                                   #
    ...
    x_unc_p <- x_unc_p[-nas]                                                   #
                                                                               ...
                                                                               #
  if(length(x_dat) == 0){                                                      #
    print(paste0("ERROR: 'x_dat' and 'y_dat' are empty. There is no data to ", #
                 "plot. Returning NULL."))                                     #
    return(NULL)                                                               #
  }                                                                            #
  if(length(str_detect(no_fit_plot_output_path, ".png")) == 0){                #
    print(paste0("ERROR: the file extinsion for 'no_fit_plot_output_path' ",   #
                 "must be '.png'. Returning NULL."))                           #
    return(NULL)                                                               #
  }                                                                            #
                                                                               #
  if(log_x_bool){                                                              #
    x_unc_p <- abs(log10(x_dat + x_unc_p) - log10(x_dat))                      #
    x_unc_n <- abs(log10(x_dat - x_unc_n) - log10(x_dat))                      #
    x_dat <- log10(x_dat)                                                      #
                                                                               #
    x_inf <- which(is.infinite(x_dat), arr.ind = T)                            #
    x_na <- which(is.na(x_dat), arr.ind = T)                                   #
    x_nas <- union(x_inf, x_na)                                                #
                                                                               #
    if(length(x_nas) != 0){                                                    #
      x_dat <- x_dat[-x_nas]                                                   #
      x_unc_p <- x_unc_p[-x_nas]                                               #
      x_unc_n <- x_unc_n[-x_nas]                                               #
      y_dat <- y_dat[-x_nas]                                                   #
      y_unc_p <- y_unc_p[-x_nas]                                               #
      y_unc_n <- y_unc_n[-x_nas]                                               #
    }                                                                          #
  }                                                                            #
  if(log_y_bool){                                                              #
    y_unc_p <- abs(log10(y_dat + y_unc_p) - log10(y_dat))                      #
    y_unc_n <- abs(log10(y_dat - y_unc_n) - log10(y_dat))                      #
    y_dat <- log10(y_dat)                                                      #
                                                                               #
    y_inf <- which(is.infinite(y_dat), arr.ind = T)                            #
    y_na <- which(is.na(y_dat), arr.ind = T)                                   #
    y_nas <- union(y_inf, y_na)                                                #
                                                                               #
    if(length(y_nas) != 0){                                                    #
      x_dat <- x_dat[-y_nas]                                                   #
      x_unc_p <- x_unc_p[-y_nas]                                               #
      x_unc_n <- x_unc_n[-y_nas]                                               #
      y_dat <- y_dat[-y_nas]                                                   #
      y_unc_p <- y_unc_p[-y_nas]                                               #
      y_unc_n <- y_unc_n[-y_nas]                                               #
    }                                                                          #
  }                                                                            #
                                                                               #
  print(paste0("* Plotting ", x_lab, " vs ", y_lab, "... *"))                  #
  if(is.na(max_x)){                                                            #
    max_x <- max(x_dat + x_unc_p, na.rm = T)                                   #
                                                                               #
    if(max_x < 0){                                                             #
      max_x <- max_x * (1 - lim_marg_x)                                        #
    }else{                                                                     #
      if(max_x == 0){                                                          #
        max_x <- lim_marg_x                                                    #
      }else{                                                                   #
  ...
  if(is.na(min_x)){                                                            #
    min_x <- min(x_dat - x_unc_n, na.rm = T)                                   #
                                                                               #
    if(min_x < 0){                                                             #
      min_x <- min_x * (1 + lim_marg_x)                                        #
    }else{                                                                     #
      if(min_x == 0){                                                          #
        min_x <- -lim_marg_x                                                   #
      }else{                                                                   #
  ...
  if(is.na(max_y)){                                                            #
    max_y <- max(y_dat + y_unc_p, na.rm = T)                                   #
                                                                               #
    if(max_y < 0){                                                             #
      max_y <- max_y * (1 - lim_marg_y)                                        #
    }else{                                                                     #
      if(max_y == 0){                                                          #
        max_y <- lim_marg_y                                                    #
      }else{                                                                   #
  ...
  if(is.na(min_y)){                                                            #
    min_y <- min(y_dat - y_unc_n, na.rm = T)                                   #
                                                                               #
    if(min_y < 0){                                                             #
      min_y <- min_y * (1 + lim_marg_y)                                        #
    }else{                                                                     #
      if(min_y == 0){                                                          #
        min_y <- -lim_marg_y                                                   #
      }else{                                                                   #
  ...
  y_lims <- c(min_y, max_y)                                                    #
                                                                               #
  png(file = no_fit_plot_output_path, width = png_width, height = png_height)  #
  if(log_x_bool | log_y_bool){                                                 #
    matplot(x = NA, y = NA, cex = plot_cex, xlab = x_lab, ylab = y_lab,        #
            xlim = x_lims, ylim = y_lims, axes = F)                            #
  }else{                                                                       #
    matplot(x = NA, y = NA, cex = plot_cex, xlab = x_lab, ylab = y_lab,        #
            xlim = x_lims, ylim = y_lims)                                      #
  }                                                                            #
  if(arrow_bool){                                                              #
    arrows(x_dat, y0 = y_dat - y_unc_n, y1 = y_dat + y_unc_p, code = 3,        #
           length = 0.02, angle = 90, lty = arrow_typ, col = arrow_col)        #
    arrows(x0 = x_dat - x_unc_n, y_dat, x1 = x_dat + x_unc_p, code = 3,        #
           length = 0.02, angle = 90, lty = arrow_typ, col = arrow_col)        #
  }                                                                            #
  points(x = x_dat, y = y_dat, type = plot_type, pch = plot_pch,               #
         lty = plot_lty, cex = plot_cex)                                       #
                                                                               #
  if(log_x_bool | log_y_bool){                                                 #
    x_ticks <- pretty(x_lims, 7)                                               #
    y_ticks <- pretty(y_lims, 7)                                               #
                                                                               #
    if(log_x_bool){                                                            #
      x_tick_labs <- format(10^x_ticks, digits = 2)                            #
    }else{                                                                     #
      x_tick_labs <- paste0(x_ticks)                                           #
    }                                                                          #
    if(log_y_bool){                                                            #
      y_tick_labs <- format(10^y_ticks, digits = 2)                            #
    }else{                                                                     #
      y_tick_labs <- paste0(y_ticks)                                           #
    }                                                                          #
    axis(1, at = x_ticks, labels = x_tick_labs, tick = T)                      #
    axis(2, at = y_ticks, labels = y_tick_labs, tick = T)                      #
  }                                                                            #
  dev.off()                                                                    #
                                                                               #
  print(paste0("* A plot of ", x_lab, " vs ", y_lab, " has been saved to ",    #
               no_fit_plot_output_path, " *"))                                 #
                                                                               #
  if(fit_bool){                                                                #
    print(paste0("* Fitting linear model to ", x_lab, " vs ", y_lab, "... *")) #
                                                                               #
    if(is.null(fit_plot_output_path) |                                         #
       length(str_detect(fit_plot_output_path, ".png")) == 0){                 #
      fit_plot_output_path <- paste0(                                          #
        strsplit(no_fit_plot_output_path, "[.png]"), "_fitted.png")            #
    }                                                                          #
                                                                               #
    # Linear model                                                             #
    lin_par_lab <- c("b", "m")                                                 #
    lin_par_list <- c(lin_par_lab, "cor", "Nresd", "R²")                       #
    lin_par_len <- length(lin_par_lab)                                         #
    lin_par_list_len <- length(lin_par_list)                                   #
                                                                               #
    ...
    lin_formula <- Y ~ b + m * X                                               #
                                                                               #
    n_sel <- length(x_dat)                                                     #
    df_sel <- n_sel - lin_par_len                                              #
                                                                               #
    start_b <- y_dat[which(x_dat == min(x_dat), arr.ind = T)]                  #
    start_m <- median(y_dat / x_dat)                                           #
    starters <- list(b = start_b, m = start_m)                                 #
                                                                               #
    y_unc <- (y_unc_n + y_unc_p) / 2                                           #
    x_unc <- (x_unc_n + x_unc_p) / 2                                           #
                                                                               #
    data_fr <- NULL                                                            #
    data_fr$Y <- y_dat                                                         #
    data_fr$X <- x_dat                                                         #
    ...
    data_fr$se.X <- x_unc                                                      #
                                                                               #
    lin_model <- tryCatch(deming(Y ~ X, data = data_fr,                        #
                                 xstd = se.X, ystd = se.Y),                    #
                          error = function(e){return(TRUE)},                   #
                          warning = function(w){return(TRUE)})                 #
                                                                               #
    lin_model_stat <- array(NA, dim = c(lin_par_list_len, stat_len),           #
                            dimnames = list(lin_par_list, statistics))         #
                                                                               #
    # Saving model parameters                                                  #
    if(!is.logical(lin_model)){                                                #
      m_pars <- lin_model$coefficients                                         #
      sd_pars <- lin_model$se                                                  #
                                                                               #
      lin_model_stat[1:lin_par_len, "mean"] <- m_pars                          #
      lin_model_stat[1:lin_par_len, "sd"] <- sd_pars                           #
                                                                               #
      model_pred <- Power_law(x_dat, c(m_pars, 0, 1))                          #
      model_sd <- sqrt(sd_pars[1]^2 + (x_dat * sd_pars[2])^2 +                 #
                         (m_pars[2] * x_unc)^2)                                #
                                                                               #
      lin_model_stat["cor", "mean"] <- cor(y_dat, model_pred,                  #
                                           use = "pairwise.complete.obs")      #
      lin_model_stat["Nresd", "mean"] <-                                       #
        median((abs((y_dat - model_pred) / y_dat) * 100), na.rm = T)           #
                                                                               #
      w_RSS_sigma <- 1 / (model_sd^2 + y_unc^2)                                #
      w_RSS_sigma_tot <- sum(w_RSS_sigma, na.rm = T)                           #
      w_rss <- w_RSS_sigma / w_RSS_sigma_tot                                   #
      TSS <- sum((y_dat - mean(y_dat))^2 + y_unc^2, na.rm = T)                 #
      RSS <- sum(w_rss * (y_dat - model_pred)^2, na.rm = T)                    #
      wR2 <- 1 - RSS / TSS                                                     #
      adjR2 <- 1 - (1 - wR2) * (n_sel - 1) / (df_sel - 1)                      #
                                                                               #
      lin_model_stat["R²", "mean"] <- adjR2                                    #
    }                                                                          #
                                                                               #
    if(df_sel > 0){                                                            #
      print(paste0("* Plotting fitted linear model of ", x_lab, " vs ",        #
                   y_lab, "... *"))                                            #
                                                                               #
      x_seq <- seq(min_x, max_x, length.out = 1000)                            #
                                                                               #
      if(!is.logical(lin_model)){                                              #
        fit_leg <- paste0("m = ",                                              #
                          format(lin_model_stat["m", "mean"], digits = 2),     #
                          " +- ", format(lin_model_stat["m", "sd"], digits =2),#
                          ",\n b = ",                                          #
                          format(lin_model_stat["b", "mean"], digits = 3),     #
                          " +- ", format(lin_model_stat["b", "sd"], digits =2),#
                          ",\n corr = ",                                       #
                          format(lin_model_stat["cor", "mean"], digits = 2),   #
                          ",\n Nresd = ",                                      #
                          format(lin_model_stat["Nresd", "mean"], digits = 2), #
                          "%,\n R² = ",                                        #
                          format(lin_model_stat["R²", "mean"], digits = 2))    #
                                                                               #
        if(log_x_bool){                                                        #
          x_lims <- c(min_x, max_x + abs(max_x - min_x) / 1.5)                 #
        }else{                                                                 #
          x_lims <- c(min_x, max_x + abs(max_x - min_x) / 3)                   #
        }                                                                      #
                                                                               #
        png(file = fit_plot_output_path, width = png_width, height =png_height)#
                                                                               #
        if(log_x_bool | log_y_bool){                                           #
          matplot(x = NA, y = NA, cex = plot_cex, xlab = x_lab, ylab = y_lab,  #
                  xlim = x_lims, ylim = y_lims, axes = F)                      #
          ...
          matplot(x = NA, y = NA, cex = plot_cex, xlab = x_lab, ylab = y_lab,  #
                  xlim = x_lims, ylim = y_lims)                                #
                                                                               ...
                                                                               #
        pred_m <- m_pars[1] + m_pars[2] * x_seq                                #
                                                                               #
        lines(x_seq, pred_m, lty = fit_lty, col = fit_col, lwd = fit_lwd)      #
                                                                               #
        pred_sd <- sqrt(sd_pars[1]^2 + (x_seq * sd_pars[2])^2)                 #
                                                                               #
        lines(x_seq, pred_m + pred_sd, lty = fit_unc_lty, col = fit_unc_col,   #
              lwd = fit_unc_lwd)                                               #
        lines(x_seq, pred_m - pred_sd, lty = fit_unc_lty, col = fit_unc_col,   #
              lwd = fit_unc_lwd)                                               #
                                                                               #
        ...
        if(is.na(leg_ypos)){                                                   #
          leg_ypos <- max_y                                                    #
                                                                               ...
                                                                               #
        legend(leg_xpos, leg_ypos, legend = fit_leg, col = fit_col,            #
               lty = fit_lty, lwd = fit_lwd, bty = fit_bty)                    #
                                                                               #
        if(arrow_bool){                                                        #
          arrows(x_dat, y0 = y_dat - y_unc_n, y1 = y_dat + y_unc_p, code = 3,  #
                 length = 0.02, angle = 90, lty = arrow_typ, col = arrow_col)  #
        ...
        dev.off()                                                              #
                                                                               #
        print(paste0("* A plot of ", x_lab, " vs ", y_lab,                     #
                     " fitted by a linear model has been saved to ",           #
                     fit_plot_output_path, " *"))                              #
      }else{                                                                   #
        print(paste0("** Fitting process was unsuccessfull. ",                 #
                     "No extra plot was saved. **"))                           #
      }                                                                        #
    }else{                                                                     #
      print(paste0("** Insufficient degrees of freedom. Can not fit model. ",  #
                   "No extra plot was saved. **"))                             #
    }                                                                          #
  }                                                                            #
}                                                                              #
...
################################################################################
# Calculates 'n_samp' data pair KS-tests while sampling 'x_dat' and 'y_dat'    #
# from their respective uncertainties. In the end it outputs the mean and sd   #
# of the D and p-value statistics of all those tests.                          #
# If 'norm_bool' is TRUE 'x_dat' and 'y_dat' are previously normalized         #
# according to 'norm_type'.                                                    #
# If 'norm_type' is "Z" the data will be transformed according to              #
# z = (data - mean(data)) / var(data)                                          #
# If 'norm_type' is "U" the data will be transformed according to              #
# u = (data - min(data)) / max((data - min(data)))                             #
# If 0 < 'mp' < 1, the function will provided completion notices at intervals  #
# of 'mp' %.                                                                   #
################################################################################
robust_ks_test <- function(x_dat, y_dat, x_unc, y_unc, norm_bool = T,          #
                           norm_type = "Z", n_samp = 1000, mp = .25,           #
                           pro_type = "HF"){                                   #
  if((length(x_dat) != length(x_unc)) & (length(x_unc) != 1)){                 #
    print(paste0("%% ERROR: 'x_dat' and 'x_unc' must either have the same len",#
                 "gth or 'x_unc' must have length 1. 'x_dat' has length ",     #
                 length(x_dat), " and 'x_unc' has length ", length(x_unc),     #
                 ". Returning NULL."))                                         #
    return(NULL)                                                               #
  }                                                                            #
  if((length(y_dat) != length(y_unc)) & (length(y_unc) != 1)){                 #
    print(paste0("%% ERROR: 'y_dat' and 'y_unc' must either have the same len",#
                 "gth or 'y_unc' must have length 1. 'y_dat' has length ",     #
                 length(y_dat), " and 'y_unc' has length ", length(y_unc),     #
                 ". Returning NULL."))                                         #
    return(NULL)                                                               #
  }                                                                            #
                                                                               #
  if(length(x_unc) == 1){                                                      #
    x_unc <- rep(x_unc, length(x_dat))                                         #
  }                                                                            #
  if(length(y_unc) == 1){                                                      #
    y_unc <- rep(y_unc, length(y_dat))                                         #
  }                                                                            #
                                                                               #
  if(length(pro_type) != 1){                                                   #
    pro_type <- "LS"                                                           #
  }                                                                            #
  if(pro_type == "hf"){                                                        #
    pro_type <- "HF"                                                           #
  ...
  ly <- length(y_dat)                                                          #
  if((lx == 0) | (ly == 0)){                                                   #
    print(paste0("%% ERROR: one of the variables holds no information. ",      #
                 "'y_dat' has ", ly, " positions with information, and 'x_dat",#
                 "' has ", lx, " positions with information. Returning NULL."))#
    return(NULL)                                                               #
  }                                                                            #
                                                                               #
  if(norm_bool){                                                               #
    ...
    if(norm_type == "u"){                                                      #
      norm_type <- "U"                                                         #
                                                                               ...
                                                                               #
    if((norm_type != "U") & (norm_type != "Z")){                               #
      print(paste0("%% ERROR: invalid 'norm_type' = ", norm_type, ". 'norm_ty",#
                   "pe' must be either 'Z' for z-transform normalization, or ",#
                   "'U' for [0;1] normalization. Returning NULL."))            #
      return(NULL)                                                             #
    }                                                                          #
                                                                               #
    if(norm_type == "Z"){                                                      #
      print("%% Performing z-normalization to 'x_dat' and 'y_dat' %%")         #
      b <- mean(x_dat)                                                         #
      unc_b <- unc_mean(x_dat, x_unc, dims2med = 0)                            #
                                                                               #
      sd <- sd(x_dat)                                                          #
      unc_sd <- sd / sqrt(2 * lx - 2)                                          #
      s <- sd^2                                                                #
      unc_s <- sqrt((2 * sd * unc_sd)^2)                                       #
                                                                               #
      x_dat <- (x_dat - b) / s                                                 #
      x_unc <- sqrt((x_unc / s)^2 + (unc_b / s)^2 + (x_dat / s * unc_s)^2)     #
                                                                               #
      b <- mean(y_dat)                                                         #
      unc_b <- unc_mean(y_dat, y_unc, dims2med = 0)                            #
                                                                               #
      sd <- sd(y_dat)                                                          #
      unc_sd <- sd / sqrt(2 * ly - 2)                                          #
      s <- sd^2                                                                #
      unc_s <- sqrt((2 * sd * unc_sd)^2)                                       #
                                                                               #
      y_dat <- (y_dat - b) / s                                                 #
      y_unc <- sqrt((y_unc / s)^2 + (unc_b / s)^2 + (y_dat / s * unc_s)^2)     #
    }else{                                                                     #
      print("%% Performing unif [0;1]-normalization to 'x_dat' and 'y_dat' %%")#
      min_dat <- min(x_dat)                                                    #
      mins <- which(x_dat == min_dat, arr.ind = T)                             #
      unc_min <- mean(x_unc[mins])                                             #
                                                                               #
      new_x_dat <- x_dat - min_dat                                             #
      ...
      x_unc <- unc_div(new_x_dat, max_dat, new_x_unc, unc_max, x_dat)          #
                                                                               #
      min_dat <- min(y_dat)                                                    #
      mins <- which(y_dat == min_dat, arr.ind = T)                             #
      unc_min <- mean(y_unc[mins])                                             #
                                                                               #
      new_y_dat <- y_dat - min_dat                                             #
      ...
      y_unc <- unc_div(new_y_dat, max_dat, new_y_unc, unc_max, y_dat)          #
                                                                               ...
                                                                               #
  if(pro_type == "HF"){                                                        #
    n_samp_x <- array(NA, dim = c(lx, n_samp))                                 #
    n_samp_y <- array(NA, dim = c(ly, n_samp))                                 #
                                                                               #
    for(x in 1:lx){                                                            #
      n_samp_x[x,] <- rnorm(n_samp, x_dat[x], x_unc[x])                        #
    }                                                                          #
    for(y in 1:ly){                                                            #
      n_samp_y[y,] <- rnorm(n_samp, y_dat[y], y_unc[y])                        #
    }                                                                          #
  }                                                                            #
                                                                               #
  ks_results <- array(NA, dim = c(2, 2), dimnames = list(c("D", "p-value"),    #
                                                         c("mean", "sd")))     #
  ks_samp_results <- array(NA, dim = c(n_samp, 2),                             #
                           dimnames = list(NULL, c("D", "p-value")))           #
                                                                               #
  print(paste0("% Starting to perform KS-test with ", n_samp, " samples %"))   #
  k <- 1                                                                       #
  for(samp in 1:n_samp){                                                       #
    if(samp == (round(n_samp * k * mp + 1))){                                  #
      print(paste0("%% KS-test is at ", k * mp * 100, "% %%"))                 #
      k <- k + 1                                                               #
    }                                                                          #
                                                                               #
    if(pro_type == "LS"){                                                      #
      x_samp <- NULL                                                           #
      for(l in 1:lx){                                                          #
        x_samp <- c(x_samp, rnorm(1, x_dat[l], x_unc[l]))                      #
      }                                                                        #
                                                                               #
      y_samp <- NULL                                                           #
      for(l in 1:ly){                                                          #
        y_samp <- c(y_samp, rnorm(1, y_dat[l], y_unc[l]))                      #
      }                                                                        #
    }else{                                                                     #
      x_samp <- n_samp_x[,samp]                                                #
      y_samp <- n_samp_y[,samp]                                                #
    }                                                                          #
                                                                               #
    t_ks <- ks.test(x_samp, y_samp)                                            #
    ks_samp_results[samp, "D"] <- t_ks$statistic                               #
    ks_samp_results[samp, "p-value"] <- t_ks$p.value                           #
  }                                                                            #
  print(paste0("% Finished KS-test with ", n_samp, " samples. ",               #
               "Calculating overall statistics. %"))                           #
                                                                               #
  ks_results["D", "mean"] <- mean(ks_samp_results[,"D"], na.rm = T)            #
  ks_results["D", "sd"] <- sd(ks_samp_results[,"D"], na.rm = T)                #
  ks_results["p-value", "mean"] <- mean(ks_samp_results[,"p-value"], na.rm = T)#
  ks_results["p-value", "sd"] <- sd(ks_samp_results[,"p-value"], na.rm = T)    #
                                                                               #
  return(ks_results)                                                           #
}                                                                              #
################################################################################