...
print("Provide the path to the python version to be used: ")                   #
py_path <- edit(py_path)                                                       #
                                                                               #
obj <- "NGC-7721"                                                              #
...
input_folder <- edit(input_folder)                                             #
                                                                               #
std_path <- paste0(input_folder, "STD/")                                       #
print("Provide a path to move the STD files to: ")                             #
std_path <- edit(std_path)                                                     #
if(!dir.exists(std_path)){                                                     #
  mkdir(std_path)                                                              #
}                                                                              #
                                                                               #
mv_flag <- 1                                                                   #
                                                                               #
if(input_folder == std_path){                                                  #
  mv_flag <- 0                                                                 #
}                                                                              #
                                                                               #
prod_path <- paste0(og_folder, "Processed_STD/")                               #
if(!dir.exists(prod_path)){                                                    #
  mkdir(prod_path)                                                             #
}                                                                              #
rm(og_folder)                                                                  #
                                                                               #
mrg_path <- paste0(prod_path, "Merged_Standards/")                             #
if(!dir.exists(mrg_path)){                                                     #
  mkdir(mrg_path)                                                              #
}                                                                              #
                                                                               #
beam_path <- paste0(prod_path, "STD_Beams/")                                   #
if(!dir.exists(beam_path)){                                                    #
  mkdir(beam_path)                                                             #
}                                                                              #
                                                                               #
cr_path <- paste0(prod_path, "STD_Cosmic_Rays/")                               #
...
dimsCH <- c(2048, 1034)                                                        #
                                                                               #
#Create list of pathList indexes for STDs fits                                 #
std_ind <- get_fits_header_key_index(types_arr, pathList, dimsCH, "STD")       #
stdsCount <- length(std_ind)                                                   #
rm(types_arr)                                                                  #
                                                                               #
stopifnot("No STD files in specified folder."= (stdsCount != 0))               #
                                                                               #
print("Checking if files are paired...")                                       #
stopifnot("At least one file is not paired."= (stdsCount %% 2) == 0)           #
print("All files are paired.")                                                 #
                                                                               #
#Determining bands in which stds are available                                 #
...
angs <- 1:ang_n                                                                #
bands_arr <- get_fits_header_list_str(pathList[std_ind], FILTER)               #
angs_arr <- get_fits_header_list_num(pathList[std_ind], HWP_ANG)               #
                                                                               #
...
for(o in 1:stdsCount){                                                         #
  ...
  if(angs_arr[o] == 67.5){                                                     #
    ...
    angs_arr[o] <- ang_str[5]                                                  #
                                                                               ...
                                                                               #
  print(paste0("STD #", o, " is in ", bands_arr[o], " band, observed with ",   #
               "the HWP at an angle of ", angs_arr[o], fin_str))               #
  rm(fin_str)                                                                  #
                                                                               #
  if(o == stdsCount){                                                          #
                                                                               #
    warning_HWP_1 <- paste0("WARNING: the number of files at each HWP angle i",#
                            "s either not the same, or zero. There are ",      #
                            ang_count["0"], " files at 0º, ",                  #
                            ang_count["22.5"], " files at 22.5º, ",            #
                            ang_count["45"], " files at 45º and ",             #
                            ang_count["67.5"], " files at 67.5º.")             #
    if(!(ang_count["0"] == ang_count["22.5"] &                                 #
         ang_count["0"] == ang_count["45"] &                                   #
         ang_count["0"] == ang_count["67.5"] &                                 #
         ang_count["0"] != 0)){                                                #
                                                                               #
      stopifnot(ang_count["0"] != 0, ang_count["22.5"] != 0,                   #
                ang_count["45"] != 0, ang_count["67.5"] != 0)                  #
                                                                               #
      print(warning_HWP_1)                                                     #
    }                                                                          #
...
for(k in bands){                                                               #
  ...
  for(l in angs){                                                              #
    ...
    length_ind <- length(temp_ind)                                             #
                                                                               #
    end_char <- "º"                                                            #
    if(ang_str[l] == "Phot") end_char <- ""                                    #
    if(length_ind == 0 || length_ind %% 2 != 0){                               #
      print(paste0("Either no STDs were found or the ones found are not pa",   #
                   "ired for the filter & angle subset: band = ", band_str[k], #
                   ", angle = ", ang_str[l], end_char))                        #
      next                                                                     #
    }                                                                          #
                                                                               #
    for(o in seq(1, length_ind, 2)){                                           #
      ...
      print(paste0("Loading files ", o, " and ", o + 1, " of ", length_ind,    #
                   " for band ", band_str[k], ", angle ", ang_str[l], end_char)#
      )                                                                        #
                                                                               #
      ...
      name_ch1 <- strsplit(fileList[std_ind[temp_ind[o]]],                     #
                           ".[0-9]+.fits")[[1]][1]                             #
      name_ch2 <- strsplit(fileList[std_ind[temp_ind[o + 1]]],                 #
                           ".[0-9]+.fits")[[1]][1]                             #
                                                                               #
      #Loading one chip of a pair to figure to which CCD it corresponds        #
      temp <- readFITS(pathList[std_ind[temp_ind[o]]])                         #
                                                                               #
      ...
      if(chip == "CHIP1"){                                                     #
        ch1 <- temp                                                            #
        ch2 <- readFITS(pathList[std_ind[temp_ind[o + 1]]])                    #
        ch1_src <- fileList[std_ind[temp_ind[o]]]                              #
        ch2_src <- fileList[std_ind[temp_ind[o + 1]]]                          #
      }else{                                                                   #
        ch1 <- readFITS(pathList[std_ind[temp_ind[o + 1]]])                    #
        ch2 <- temp                                                            #
        ch1_src <- fileList[std_ind[temp_ind[o + 1]]]                          #
        ch2_src <- fileList[std_ind[temp_ind[o]]]                              #
      }                                                                        #
      rm(temp, chip)                                                           #
                                                                               #
      #Moving source files to a new folder                                     #
      if(mv_flag){                                                             #
        file.copy(pathList[std_ind[temp_ind[o]]],                              #
                  paste0(std_path, fileList[std_ind[temp_ind[o]]]))            #
        file.remove(pathList[std_ind[temp_ind[o]]])                            #
        file.copy(pathList[std_ind[temp_ind[o + 1]]],                          #
                  paste0(std_path, fileList[std_ind[temp_ind[o + 1]]]))        #
        file.remove(pathList[std_ind[temp_ind[o + 1]]])                        #
      }                                                                        #
                                                                               #
      print(paste0("STD set #", number, " for filter ", band_str[k],           #
                   " and HWP angle ", ang_str[l], "º has been loaded: ", o + 1,#
                   "/", length_ind))                                           #
      ...
      rm(date_name)                                                            #
                                                                               #
      #Preparing merged chips std files' header                                #
      ...
      temp_head <- addKwv('COSM-CTR', cont_str, "Contrast used for CR removal",#
                          temp_head)                                           #
      temp_head <- addKwv('FILETYP1', 'STANDARD', "File type", temp_head)      #
      temp_head <- addKwv('FILETYP2', 'MIXED BEAMS', "File sub-type",          #
                          temp_head)                                           #
      temp_head <- addKwv('SOURCE', 'process_STD.R',                           #
      ...
      rm(ref_mrg)                                                            # #
                                                                             # #
      #Creating Merged std file                                              # #
      writeFITSim(mrg_data, file = paste0(mrg_path, fits_name, "-Merged-STD",# #
                                          ".fits"), crpixn = crpix,          # #
                  crvaln = crval, ctypen = ctype, header = temp_head)        # #
                                                                             # #
      print(paste0("Merged chips, debiased STD file number ", number, " ",   # #
                   "for filter ", band_str[k], " and HWP angle ", ang_str[l],# #
                   "º has been created."))                                   # #
      ##########################################################################
                                                                               #
      #----------------------------------------------------------------------# #
      ...
      rm(lacosm_cmd)                                                         # #
                                                                             # #
      #Creating CR removed STD file                                          # #
      writeFITSim(mrg_data, file = paste0(mrg_path, fits_name,               # #
                                          "-Merged-STD-no-CRs.fits"),        # #
                  crpixn = crpix, crvaln = crval, ctypen = ctype,            # #
                  header = temp_head)                                        # #
      writeFITSim(py$cr_map, paste0(cr_path, fits_name,                      # #
                                    "-Merged-STD-CR.fits"))                  # #
                                                                             # #
      print(paste0("Merged chips, debiased, Cosmic Ray removed STD file ",   # #
                   "number ", number, " for filter ", band_str[k], " and HW",# #
                   "P angle ", ang_str[l], "º has been created."))           # #
      ##########################################################################
                                                                               #
      #----------------------------------------------------------------------# #
                                                                               #
      ...
      for(m in seq(1, length(ord_ind), 2)){                                  # #
        ...
        dy <- round(median(par_dy["a", band_str[k]] +                        # #
                             par_dy["b", band_str[k]] * (li_o:ls_o - 450)^2))# #
        dx <- 0                                                              # #
                                                                             # #
...
rm(std_path, mrg_path, beam_path, cr_path, k, EXPTIME, REFX, REFY, VALX, CHIP, #
   VALY, SIZEX, SIZEY, TYPEX, TYPEY, BINX, BINY, DATE, X_GAP, Y_GAP, HWP_ANG,  #
   FILTER, ANG_GAP, PRESCAN, OVERSCAN, ang_str, band_str, pathList, cr_t_str,  #
   bias1_M, bias2_M, dimsCH, par_dy, cont_str, std_ind, bands, angs, fileList, #
   ind_phot, ind_0, ind_22, ind_45, ind_68, run_date, mv_flag, N, sat, GAIN,   #
   RON, warning_HWP_1)                                                         #
                                                                               #
if(flat_flag == 1){                                                            #
